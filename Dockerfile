...
WORKDIR /workspace/LLMs-from-scratch
ENV HF_HOME=/workspace/.cache/huggingface \
    ...
    PYTHONUNBUFFERED=1 \
    HOME=/workspace
RUN mkdir -p "${HF_HOME}" /workspace/LLMs-from-scratch && chown -R ${UID}:${GID} /workspace
...
# --- Install Jupyter kernel for the venv (as root before switching user) ---
RUN set -eux; \
    if [ -f .venv/bin/python ]; then \
        .venv/bin/python -m ipykernel install --name=llms-venv --display-name="Python (LLMs venv)"; \
    fi
...
EXPOSE 8888
CMD ["/workspace/LLMs-from-scratch/.venv/bin/jupyter", "lab", "--ip", "0.0.0.0", "--no-browser"]
